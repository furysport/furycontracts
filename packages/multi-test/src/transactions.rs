...
#[cfg(feature = "iterator")]
/// The BTreeMap specific key-value Record reference type, as returned by BTreeMap<Vec<u8>, T>::range.
...
enum Op {
    /// represents the `Set` operation for setting a key-value Record in storage
    Set {
        key: Vec<u8>,
        value: Vec<u8>,
    },
